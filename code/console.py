...
def prompt(*args):
    return input(colors.BLUE + fmt(args) + colors.END)
def log(*args):
    print(fmt(args))
def info(*args):
    print(colors.DIM + "\t" + fmt(args) + colors.END)
def progressBar(prop):
    maxWidth = 50
    width = ceil(prop * maxWidth)
    progress("[" + ":" * width + "." * (maxWidth - width) + "]")
    if (prop == 1.0):
        print()
def progress(*args):
    print(colors.DIM + "\t", fmt(args), colors.END, end='\r')
def debug(*args):
...
def stats(matrix, name=""):
    if name:
        name += " : "
    print(colors.DK_BLUE + "{}{} {} [min {}, max {} mean {}]".format(name, matrix.dtype, matrix.shape, matrix.min(), matrix.max(), matrix.mean()) + colors.END)
def time(key):
